...
// DEMO: the spreadsheet webhook is deliberately absent. This copy must not
// be able to write into the couple's real guest list, and an unused constant
// is still a live endpoint sitting in public JavaScript.
...
document.getElementById('rsvp-submit-yes').addEventListener('click', async () => {
    ...
    // DEMO: minted in the browser instead of by /api/wedding/rsvp. Same
    // alphabet and length as a real ticket (no I, L, O or U), so the QR and the
    // scanner scene demo exactly as delivered — the code just resolves to
    // nothing, because nothing was written anywhere.
    let ticketCode = '';
    {
    ...
    // The confirmation path below is unchanged from the delivered site, so the
    // demo looks and feels identical. The only difference is that no request
    // leaves the browser.
