...
// ── Wedding invite (opeyemianduriel.aevaia.com) ───────────────────────────────

export const weddingRsvpSchema = z.object({
  name:  z.string().trim().min(1, "Name is required").max(100, "Name must be 100 characters or less"),
  // Kept loose on purpose: the invite collects international numbers via
  // intl-tel-input, which already validates format client-side. Server-side we
  // only guard length so a valid Nigerian, UK, or US number is never rejected.
  phone: z.string().trim().min(5, "Phone number is required").max(32, "Phone number is too long"),
  email: z.string().trim().max(200).optional().or(z.literal("")),
});

// Ticket codes are generated by the server (see app/api/wedding/rsvp/route.ts).
